...
/**
 * Scheduled function that runs every Monday at 2:00 AM Eastern Time
 * Cleans up expired match documents to prevent database bloat
 * This runs after new matches are generated on Friday, giving users the weekend
 * to view their matches before cleanup
 */
export const cleanupExpiredMatches = onSchedule(
  {
  ...
  async () => {
    try {
      console.log("Starting cleanup of expired matches");

      const now = new Date();
      console.log(`Current time: ${now.toISOString()}`);

      // Find all matches that have expired (expiresAt < now)
      const expiredMatchesSnapshot = await db
        .collection("weeklyMatches")
        .where("expiresAt", "<", now)
        .get();

      if (expiredMatchesSnapshot.empty) {
        console.log("No expired matches found to clean up");
        return;
      }

      console.log(
        `Found ${expiredMatchesSnapshot.size} expired match documents to delete`
      );

      // Delete expired matches in batches (Firestore batch limit is 500)
      const batchSize = 500;
      let deletedCount = 0;

      for (let i = 0; i < expiredMatchesSnapshot.docs.length; i += batchSize) {
        const batch = db.batch();
        const batchDocs = expiredMatchesSnapshot.docs.slice(i, i + batchSize);

        batchDocs.forEach((doc) => {
          batch.delete(doc.ref);
          const data = doc.data();
          const expiredDate =
            data.expiresAt?.toDate?.()?.toISOString() || "unknown";
          console.log(
            `Deleting expired match: ${doc.id} ` +
              `(user: ${data.netid}, prompt: ${data.promptId}, expired: ${expiredDate})`
          );
        });
        ...
        console.log(
          `Deleted batch ${Math.floor(i / batchSize) + 1} (${batchDocs.length} documents)`
        );
      }

      console.log(
        `Cleanup complete. Deleted ${deletedCount} expired match documents.`
      );
    } catch (error) {
